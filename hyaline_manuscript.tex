...
Determining the conformational state of G protein-coupled receptors (\gpcr{}s) from their three-dimensional structures is essential for understanding signal transduction and rational drug design. Here we present \hyaline, a geometric deep learning framework that predicts whether a \gpcr{} structure is in an active or inactive conformational state with near-perfect accuracy. \hyaline{} integrates protein language model embeddings from ESM3 with radial basis function distance encodings and biologically-informed attention mechanisms that focus on conserved activation motifs (DRY, NPxxY, CWxP). Using E(n)-equivariant graph neural networks that respect the rotational and translational symmetry of molecular structures, our model achieves an area under the receiver operating characteristic curve (\auroc) of 0.995 and accuracy of 97.6\% on a dataset of 1,590 experimentally determined \gpcr{} structures spanning four major receptor classes. Ablation studies demonstrate that each architectural component contributes to performance, with ESM3 embeddings providing the largest individual contribution. Attention weight analysis reveals that the model has learned to focus on structurally important regions including transmembrane helices 3, 5, 6, and 7 that undergo the largest conformational changes during activation. \hyaline{} provides a rapid, accurate tool for annotating \gpcr{} structural states with applications in structure-based drug discovery, cryo-EM structure interpretation, and molecular dynamics trajectory analysis. The model and code are freely available.
...
G protein-coupled receptors constitute the largest superfamily of membrane proteins in the human genome, with approximately 800 members that collectively mediate cellular responses to an extraordinary diversity of extracellular stimuli including hormones, neurotransmitters, lipids, ions, and photons\autocite{Zhang2024GPCR}. These seven-transmembrane receptors serve as molecular switches that transduce extracellular signals across the plasma membrane to activate intracellular effector proteins, most notably heterotrimeric G proteins and $\beta$-arrestins. The central role of \gpcr{}s in physiology is reflected in their status as the targets of approximately one-third of currently approved pharmaceuticals, spanning therapeutic areas from cardiovascular disease to psychiatric disorders, metabolic syndrome, and cancer\autocite{Hauser2021}.

The fundamental mechanism of \gpcr{} signaling involves ligand-induced conformational changes that propagate from the extracellular ligand-binding site to the intracellular effector coupling interface\autocite{Weis2018}. Upon agonist binding, receptors undergo characteristic structural rearrangements including rotation of transmembrane helix 6 (TM6) and opening of the intracellular cavity to accommodate G protein or arrestin binding. These conformational changes are orchestrated by a network of conserved microswitches including the DRY motif at the cytoplasmic end of TM3, the NPxxY motif in TM7, and the CWxP motif in TM6 that together function as an allosteric transmission system linking ligand binding to effector activation\autocite{Hauser2021}. Understanding the structural determinants of \gpcr{} activation has been fundamentally transformed by advances in structural biology, particularly cryo-electron microscopy, which has enabled determination of high-resolution structures of receptors in complex with diverse ligands and signaling partners.

The rapid accumulation of \gpcr{} structural data---with over 1,100 structures now deposited in the Protein Data Bank\autocite{Caniceiro2025}---has created both opportunities and challenges. On one hand, this structural information provides an invaluable resource for understanding receptor mechanisms and guiding drug design. On the other hand, the heterogeneity of experimental conditions, crystallization constructs, and ligand occupancy means that structures must be carefully annotated regarding their conformational state before meaningful comparisons can be made. While databases such as GPCRdb provide curated annotations, the manual assignment of activation states remains labor-intensive and can be subjective, particularly for structures representing intermediate states along the activation pathway.

Several computational approaches have been developed to address the challenge of \gpcr{} state classification\autocite{Caniceiro2025}. Traditional machine learning methods including random forests and gradient boosting have achieved reasonable accuracy when trained on carefully selected structural features such as inter-residue distances between activation-relevant positions. More recently, deep learning approaches including convolutional neural networks applied to molecular dynamics trajectories have demonstrated improved performance on specific receptor subtypes\autocite{Do2023}. However, existing methods typically suffer from one or more limitations: restriction to specific receptor classes (particularly class A), reliance on hand-crafted features that may not capture the full complexity of conformational states, limited interpretability regarding the structural basis of predictions, or inability to leverage the evolutionary information encoded in protein sequences.

Here we present \hyaline{}, a geometric deep learning framework that addresses these limitations through several innovations. First, we integrate representations from ESM3, a state-of-the-art protein language model, which provides rich per-residue embeddings that capture evolutionary and functional information learned from hundreds of millions of protein sequences\autocite{Lin2023}. Second, we employ E(n)-equivariant graph neural networks that respect the fundamental symmetries of molecular structures, ensuring that predictions are invariant to rotations and translations of the input coordinates. Third, we incorporate biologically-motivated attention biasing that encourages the model to focus on conserved activation motifs while remaining flexible enough to learn additional relevant features from data. Finally, we provide interpretable attention weights that reveal which structural elements contribute most to predictions, enabling biological insights beyond simple classification.

We demonstrate that \hyaline{} achieves \auroc{} of 0.995 on a diverse dataset of 1,590 \gpcr{} structures spanning classes A, B1, C, and F, substantially outperforming baseline approaches. Systematic ablation studies quantify the contribution of each architectural component. Analysis of learned attention patterns reveals that the model has discovered the importance of established activation motifs without explicit supervision, while also identifying additional structural features that contribute to state discrimination. \hyaline{} provides a powerful tool for rapid annotation of \gpcr{} structural states with applications spanning structure determination, drug discovery, and analysis of conformational ensembles from molecular dynamics simulations.
...
We assembled a comprehensive dataset of experimentally determined \gpcr{} structures from the Protein Data Bank, cross-referenced with annotations from GPCRdb to assign activation state labels (Methods). After quality filtering to remove structures with missing coordinates, incomplete transmembrane domains, or ambiguous annotations, our final dataset comprised 1,590 structures representing 162 unique \gpcr{}s from four major receptor classes (Fig.~\ref{fig:architecture}a). Class A (Rhodopsin family) receptors were most represented with 1,263 structures (79.4\%), reflecting their dominance in the Protein Data Bank. Class B1 (Secretin family) contributed 145 structures (9.1\%), Class C (Glutamate family) 94 structures (5.9\%), and Class F (Frizzled family) 34 structures (2.1\%).
...
To assess generalization across the structural diversity of \gpcr{}s, we stratified performance by receptor family (Fig.~\ref{fig:family_performance}a). Class A receptors, which comprise the majority of the dataset, showed the highest performance with \auroc{} of 0.997. Notably, performance remained strong for the less well-represented classes: Class B1 achieved \auroc{} of 0.988, Class C achieved 0.971, and Class F achieved 0.956, despite having only 34 structures in the dataset.
...
We thank the GPCRdb team for maintaining the comprehensive database that enabled this work. Computational resources were provided by institutional research computing facilities.
...
Conceptualization: A.K., H.K. Methodology: A.K. Software: A.K. Formal Analysis: A.K. Writing---Original Draft: A.K., H.K. Writing---Review \& Editing: A.K., H.K. Visualization: A.K. Supervision: H.K.

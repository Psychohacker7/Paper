...
\textbf{(a)} \hyaline{} (purple) achieves an \auroc{} of 0.991, significantly outperforming sequence-only baselines including ESM3 embeddings with linear classifier (\auroc{} = 0.852), ESM3 with MLP (\auroc{} = 0.867), and random forest models trained on hand-crafted inter-residue distance features (\auroc{} = 0.891). The 13.9 percentage point performance gap relative to the ESM3-only baseline demonstrates the critical contribution of geometric message passing in resolving conformational states that remain indistinguishable by amino acid sequence information alone. 
\textbf{(b)} Classification performance remains robust across the full range of experimental resolutions (1.5--4.0~\AA{}), confirming that \hyaline{} captures activation-discriminative features that are resolution-independent. Error bars denote 95\% confidence intervals computed via bootstrap resampling ($n = 1{,}000$).}
...
\textbf{(a)} Mean attention weights mapped to Ballesteros-Weinstein residue positions show elevated importance at the DRY (3.2$\times$ enrichment), NPxxY (2.8$\times$), and CWxP (2.4$\times$) motifs, with additional peaks at the intracellular end of TM5 (2.1$\times$) and helix 8 (1.9$\times$). 
\textbf{(b)} Distribution of attention scores at conserved motifs compared to variable regions such as the PIF motif ($p < 0.001$, permutation test with $n = 10{,}000$ resamples). The model assigns significantly elevated importance to regions known to undergo characteristic conformational changes upon activation, validating that the network has learned the underlying biophysics of receptor activation rather than superficial structural correlates or dataset-specific artifacts.}
...
\textbf{(b)} Upon activation by agonist and G protein coupling (PDB: 3SN6), this ionic lock is broken (R$^{3.50}$--E$^{6.30}$ distance: 11.2~\AA{}) as TM6 undergoes its characteristic 14~\AA{} outward displacement. Arg$^{3.50}$ rotates into the opened intracellular cavity where it coordinates the C-terminal helix of G$\alpha_s$. The elevated attention scores assigned by \hyaline{} to this region (Fig.~\ref{fig:interpretability}) confirm that the model discriminates conformational states based on this precise physical mechanism rather than superficial structural correlates.}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Functional geometry of learned representations}
    \label{fig:ext_training_a}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:ext_classes_a}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/Embeddings_BGPCRclass.pdf}
    \caption{Embedding clustering by GPCR class}
    \label{fig:ext_classes_b}
\end{subfigure}
...
\textbf{(a)} Structures cluster primarily by activation state (active versus inactive, indicated by color) rather than by sequence homology or receptor family, demonstrating that \hyaline{} has learned a generalizable ``activation axis'' that transfers across the mechanistically diverse GPCR superfamilies. 
\textbf{(b)} Secondary clustering by receptor class (Class A, B, C, F) reflects family-specific structural features, such as the distinct TM6 kinking in Class B1 receptors versus the rigid-body TM6 rotation in Class A, that are superimposed on the shared activation signature. This hierarchical organization validates the model's ability to capture both universal physics of activation and receptor-specific structural nuances.}

...
\textbf{b}, Representative active (blue) and inactive (red) structures from each receptor class, highlighting class-specific activation mechanisms. Class A ($\beta_2$AR, PDB: 3SN6/2RH1): pronounced TM6 outward movement ($\sim$14 \AA{}), ionic lock R$^{3.50}$--E$^{6.30}$ broken. Class B1 (GLP-1R, PDB: 6X18/6LN2): TM6 kinking with extracellular unwinding. Class C (mGluR5, PDB: 6N52/6N51): subtle intramolecular changes with dimer reorientation. Class F (SMO, PDB: 6O3C/5L7D): intermediate TM6 movement with TM5/TM7 contributions.
...
\centering
\vspace{4cm}
\textbf{FIGURE 6: Error Analysis and Challenging Case Studies}\\[1em]
\textit{[Figure to be generated]}
\vspace{1cm}
}}
\caption{\textbf{Error analysis reveals biological ambiguity rather than model failure.}
...
\textbf{b}, Categorization of the 39 misclassified structures by likely cause. Partially activated intermediates (agonist-bound without transducer): $n=15$. Class C receptors with distinct mechanism: $n=11$. Structures with experimental artifacts: $n=8$. Unexplained: $n=5$.
\textbf{c}, Correctly classified challenging case: muscarinic M2 receptor with negative allosteric modulator (PDB: 4MQT). Despite intermediate TM6 position, \hyaline{} correctly predicts inactive state (confidence: 0.72). Attention concentrates on the partially intact ionic lock (R$^{3.50}$--E$^{6.30}$ distance: 5.8~\AA{}) rather than the allosteric site.
\textbf{d}, Instructive misclassification: adenosine A$_{2A}$R bound to agonist CGS21680 without G protein (PDB: 2YDO), annotated as active but predicted as inactive (confidence: 0.61). Structural analysis reveals a pre-activated intermediate: TM3--TM6 C$\alpha$ distance is 12.1~\AA{} (versus 8.5~\AA{} inactive, 16.8~\AA{} active), and the connector region (P$^{5.50}$/F$^{6.44}$/I$^{3.40}$) shows partial rearrangement.}
...
\textbf{b}, Cytoplasmic view highlighting the opening of the intracellular cavity in the active state that accommodates G protein binding. Ionic lock distance (R$^{3.50}$--E$^{6.30}$): 3.5~\AA{} in inactive, 11.2~\AA{} in active.
\textbf{c}, Close-up of the DRY-NPxxY-CWxP motif triangle, showing the conformational changes in these conserved microswitches. In the active state, R$^{3.50}$ extends into the G protein-binding pocket and forms a hydrogen bond with Y$^{5.58}$.
...
\textbf{a}, Schematic diagrams of transmembrane helix movements upon activation for each class. Class A: TM6 outward rotation ($\sim$14 \AA{}), ionic lock R$^{3.50}$--E$^{6.30}$ broken. Class B1: Pronounced TM6 kinking (sharp bend at mid-helix) and extracellular unwinding, $\sim$10--18~\AA{} displacement. Class C: Minimal intramolecular changes ($\sim$2--4~\AA{}); activation primarily involves dimer reorientation, ionic lock formed by K$^{3.46}$ rather than R$^{3.50}$. Class F: Intermediate TM6 movement ($\sim$5--8~\AA{}) with additional TM5 and TM7 contributions.
\textbf{b}, Class-specific attention weight patterns learned by \hyaline{}, showing adaptation to different activation mechanisms. Class A: strong DRY/NPxxY enrichment. Class B1: attention extends to TM6 kink region. Class C: broader distribution reflecting dimer interface contributions.
...
\subsection*{Extended Data Figure 4: Intermediate State Gallery}
...
\centering
\vspace{5cm}
\textbf{EXTENDED DATA FIGURE 4: Gallery of Intermediate Conformational States}\\[1em]
\textit{[Figure to be generated]}
\vspace{1cm}
}}
\caption{\textbf{Structures with intermediate \hyaline{} confidence scores represent partially activated conformations.}
Eight representative structures with confidence scores between 0.4--0.6, shown as superpositions with canonical active (blue) and inactive (red) reference structures. For each structure: PDB code, receptor name, bound ligand, \hyaline{} confidence score, ionic lock distance (R$^{3.50}$--E$^{6.30}$ or equivalent), and TM6 displacement from inactive reference.
\textbf{a}, A$_{2A}$R + CGS21680 (agonist) without G protein (PDB: 2YDO). Confidence: 0.61. Ionic lock: 6.8~\AA{}. TM6 displacement: 3.6~\AA{}.
\textbf{b}, A$_{2A}$R + UK-432097 (agonist) without G protein (PDB: 2YDV). Confidence: 0.58. Ionic lock: 7.1~\AA{}. TM6 displacement: 4.2~\AA{}.
\textbf{c}, $\beta_1$AR + dobutamine (partial agonist) (PDB: 2Y03). Confidence: 0.52. Ionic lock: 5.4~\AA{}. TM6 displacement: 2.8~\AA{}.
\textbf{d}, M2R + iperoxo (agonist) without G protein (PDB: 4MQS). Confidence: 0.55. Ionic lock: 6.2~\AA{}. TM6 displacement: 3.4~\AA{}.
\textbf{e--h}, Additional examples from Class B1 and Class C receptors showing analogous intermediate conformations.
These structures demonstrate that intermediate confidence scores correspond to genuine conformational intermediates rather than model uncertainty, validating \hyaline{}'s ability to detect the activation continuum.}
\label{fig:ext_intermediates}
\end{figure}

\subsection*{Extended Data Figure 5: Cross-Receptor Generalization}
...
\centering
\vspace{4cm}
\textbf{EXTENDED DATA FIGURE 5: Cross-Receptor Generalization Analysis}\\[1em]
\textit{[Figure to be generated]}
\vspace{1cm}
}}
\caption{\textbf{Leave-one-receptor-out analysis demonstrates generalization to novel receptors.}
\textbf{a}, Heatmap showing \auroc{} when training on all receptors except one (rows) and testing on the held-out receptor (columns). Values $>$0.95 (green) indicate successful generalization; values $<$0.85 (red) indicate generalization failure. Most receptor pairs show strong cross-generalization.
\textbf{b}, Correlation between sequence identity to nearest training receptor and test performance. Receptors with $<$30\% sequence identity to any training receptor maintain \auroc{} $>$0.92, demonstrating generalization beyond sequence similarity.
\textbf{c}, Hierarchical clustering of receptors based on learned representations, colored by receptor family. Clustering reflects evolutionary relationships, indicating that \hyaline{} has learned family-specific features.
\textbf{d}, Performance stratified by number of training structures for each receptor. Receptors with $\geq$10 training structures achieve \auroc{} $>$0.98; receptors with 1--5 training structures achieve \auroc{} $\sim$0.94.}
\label{fig:ext_generalization}
\end{figure}

\subsection*{Extended Data Figure 6: Application to AI-Predicted Structures}
...
\centering
\vspace{4cm}
\textbf{EXTENDED DATA FIGURE 6: Hyaline Assessment of AI-Predicted Structures}\\[1em]
\textit{[Figure to be generated]}
\vspace{1cm}
}}
\caption{\textbf{\hyaline{} provides orthogonal assessment of AI-predicted \gpcr{} conformations.}
\textbf{a}, Comparison of \hyaline{} predictions on experimental structures versus AlphaFold 3 predictions for the same receptors. For receptors with known active and inactive experimental structures, AlphaFold 3 predictions receive intermediate confidence scores (mean: 0.62), consistent with the ambiguous conformational states reported in recent benchmarks\autocite{Xu2025}.
\textbf{b}, Calibration comparison: \hyaline{} confidence scores on experimental structures (blue) versus AlphaFold 3 predictions (orange). Predicted structures show systematic shift toward intermediate confidence, reflecting their intermediate conformational character.
\textbf{c}, Case study: $\beta_2$-adrenergic receptor. Experimental active (PDB: 3SN6) receives confidence 0.97; experimental inactive (PDB: 2RH1) receives confidence 0.04; AlphaFold 3 prediction receives confidence 0.58. Structural overlay shows AlphaFold 3 prediction intermediate between active and inactive.
\textbf{d}, Distribution of \hyaline{} confidence scores across 50 AlphaFold 3-predicted \gpcr{} structures. Unlike experimental structures (bimodal distribution), predicted structures cluster near 0.5, indicating systematic production of intermediate states.}
\label{fig:ext_alphafold}
\end{figure}
